...
nombre_archivo_excel_graficos <- paste0("001_Graf - ", modificacion01, " - ", modificacion02, ".xlsx")
nombre_archivo_power_graficos <- paste0("001_Power - Graficos - ", modificacion01, " - ", modificacion02, ".pptx")
##########################################################################################################

# nombre_archivo_excel_graficos <- "AAA.xlsx"
...
### ###     cat(modificacion03, " - PowerPoint - Grafico Interna ", GenNum(num_interno_grafico), " - Inicio",  "\n", sep="")  
  ...
  if (este_detalle != "Reveer" && este_detalle != "No Corresponde") {
   ...
   # cat("Va el... ", n, "\n")
    ...
    eval(parse(text= mi_grafico, encoding = "UTF-8"))
    dev.off()
...
#    doc_pptx <- ph_with_img(doc_pptx, src = direccion_completa, type = "body", height = 5, width = 5)
    ...
    # Version Nueva
### ###    anyplot01 <- plot_instr(mi_grafico)
 #   body_add_img(x = doc_pptx, src = direccion_completa, style = NULL, width= 5, height= 5, pos = "after")
### ###     doc_pptx <- ph_with( x = doc_pptx, anyplot01, location = ph_location_fullsize())    
#########################################################################################################
    ...
    if (length(dt_mi_grafico2) == 1) {
      ...
      # Guardamos el grafico en el Power
      ### ###      anyplot02 <- plot_instr(mi_grafico2)
      ### ###doc_pptx <- ph_with( x = doc_pptx, anyplot02, location = ph_location_fullsize())  
      
    #  doc_pptx <- add_slide(doc_pptx, layout = "Title and Content", master = "Office Theme")
      # doc_pptx <- ph_with_vg(doc_pptx, code = eval(parse(text= mi_grafico2)),
      #                        type = "body")
      #doc_pptx <- ph_with(doc_pptx, src = direccion_completa_g2, location = ph_location_fullsize(),                          type = "body")

############################################################################################################      
      # Version anterioir
    #  doc_pptx <- ph_with_img(doc_pptx, src = direccion_completa_g2, type = "body", height = 5, width = 5)
      
     # body_add_img(x = doc_pptx, src = direccion_completa_g2, width= 5, height= 5, pos = "after")
      
      # Version nueva
      # doc_pptx <- ph_with(doc_pptx, value = direccion_completa, location = ph_location_fullsize(), 
      #                     type = "body")    
############################################################################################################
    }
...
# cat("ESSSTTEEE...", "num_interno_grafico: ", num_interno_grafico, "\n")
    ...
    ### ###    cat(modificacion03, " - PowerPoint - Grafico Interna ", GenNum(num_interno_grafico), " - Fin",  "\n", sep="")
    ...
    if (este_detalle == "Reveer") {
      ...
      cat(modificacion03, " - Grafico Interna ", GenNum(num_interno_grafico), " - Fin (En Revision)", "\n", sep="")
    ...
    if (este_detalle == "No Corresponde") {
      ...
      cat(modificacion03, " - Tabla Grafico ", GenNum(num_interno_grafico),  " - Fin" , " - Generación Externa o Cancelada",  "\n", sep="")
...
### ###direccion_save_power_graficos <- paste0(mi_mega_directorio, "/", nombre_archivo_power_graficos)
### ###print(doc_pptx, target = direccion_save_power_graficos)
...
#  library(rJava)
  ...
  for (n in 1:length(tablas_graficos)) {
    ...
    # Cartel de aviso    
    cat(modificacion03, " - Excel graficos_internas:  ", n, "\n", sep = "")
  ...
  # nombre_archivo <- "001_Doc - Excel - Graficos.xlsx"
direccion_save_excel_graficos <- paste0(mi_mega_directorio, "/", nombre_archivo_excel_graficos)
  #saveWorkbook(wb_graf, direccion_save)
  openxlsx::saveWorkbook(wb_graf, direccion_save_excel_graficos, overwrite = T)
  ###
