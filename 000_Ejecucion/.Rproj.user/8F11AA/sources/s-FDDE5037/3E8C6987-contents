...
tabla3 <- input_tabla
estos_nombres <- colnames(tabla3)
estos_si <- c(" Indometacina", " Ibuprofeno EV", " Ibuprofeno Oral", " Paracetamol Endovenoso")
orden <- c(1:length(estos_nombres))
dt_orden <- rep(NA, length(estos_si))
for(k in 1:length(estos_si)) if (sum(estos_nombres == estos_si[k]) > 0) dt_orden[k] <- orden[estos_nombres == estos_si[k]]
#dt_estos <- c(9, 1, 5, 12)
dt_estos <- na.omit(dt_orden)

if (ncol(tabla3) > 1) {
tabla3 <- tabla3[,dt_estos]
tabla_fa <- tabla_fa[,dt_estos]
}
...
# cantidad_filas <- 6
# dt_3 <- order(rowSums(tabla3), decreasing = T)[c(1:cantidad_filas)]
# 
# tabla3 <- tabla3[dt_3, ]
...
if (!is.nan(tabla3[j,k])) if (tabla3[j,k] != 0) {
