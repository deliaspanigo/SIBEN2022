...
Z_peso_egreso <- rep(NA, nrow(BASE_MOD_ORIGINAL))
...
# Generacion de los valores Z de peso al alta
...
Z_peso_egreso[n] <- calculo_z

} else Z_peso_egreso[n] <- NA
...
Z_peso_egreso[Z_peso_egreso > 6] <- 6
Z_peso_egreso[Z_peso_egreso < -6] <- -6


recat_Z_peso_egreso <- Z_peso_egreso


dim(recat_Z_peso_egreso) <- c(length(recat_Z_peso_egreso), 1)
colnames(recat_Z_peso_egreso) <- "recat_Z_peso_egreso"

# Salida
data_salida <- recat_Z_peso_egreso
