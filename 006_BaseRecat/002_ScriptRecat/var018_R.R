...
# Las categorias
fenton_peso_egreso <- rep(NA, nrow(BASE_MOD_ORIGINAL))



mini <- data.frame(sexo, edad_gestacional_final, pesos_final)



# Generacion fenton peso al egreso
...
if (este_peso < p3) fenton_peso_egreso[n] <- "PEG SEVERO" else 
if (este_peso >= p3 && este_peso < p10) fenton_peso_egreso[n] <- "PEG" else 
if (este_peso >= p10 && este_peso <= p90) fenton_peso_egreso[n] <- "AEG" else 
if (este_peso > p90) fenton_peso_egreso[n] <- "GEG"
...
recat_fenton_peso_egreso <- fenton_peso_egreso

dim(recat_fenton_peso_egreso) <- c(length(recat_fenton_peso_egreso), 1)
colnames(recat_fenton_peso_egreso) <- "recat_fenton_peso_egreso"

# Salida
data_salida <- recat_fenton_peso_egreso
