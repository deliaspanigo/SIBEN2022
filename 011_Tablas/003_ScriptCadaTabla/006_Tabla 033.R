...
mini[,1] <- paste0(mini[,1] ,"- Nacimiento", sep="")

mini[,2] <- paste0(mini[,2] ,"- Alta", sep="")
  ...
  colnames(tabla_intermedia)[1] <- colnames(mini)[1]
  ...
  orden_columnas <- c(1,5,4,3,2,6)
  orden_filas <- c(4,3,2,1,5)
  
  tabla_salida <- tabla_salida[orden_filas, orden_columnas]
