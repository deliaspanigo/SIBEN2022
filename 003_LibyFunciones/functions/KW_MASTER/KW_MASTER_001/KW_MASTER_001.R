...
#   
  ...
  # Pero puede armarse para ingresar con el nombre de las variables, y así las salidas tienen el nombre de las variables.
    ...
    ANALISIS$statistic <- round(ANALISIS$statistic, input_decimales)
    ANALISIS$p.value <- ANALISIS$statistic[2]
    
    p_interno_kw <- ANALISIS$p.value
    if (p_interno_kw < 0.01) p_externo_kw <- "<<0.01" else p_externo_kw <- p_interno_kw
  ...
  # Tabla de Grupos KW
  {
    ...
    for (heaven in 1:nrow(TABLA_GRUPOS)) {
      ...
      esta_mediana <- round(esta_mediana, input_decimales)
