...
BARTLETT_MASTER <- function(input_datos=NULL, input_decimales=2, input_alfa=0.05, input_ingreso="clasic"){
  ...
  if (control_general == TRUE) {
    ...
    # Test de Homogeneidad de Bartlett
    {
      ...
      # Estadistico
      estadistico <- round(ANALISIS$ORIGINAL$statistic, input_decimales)
      ANALISIS$CONSULTORA$estadistico <- estadistico
      
      # Valor p interno
      valor_p_interno <- round(ANALISIS$ORIGINAL$p.value, input_decimales)
    ...
    # frase2_html: segun valor p
    {
      ...
      if (input_ingreso == "anova") {  
        ...
        frase2_v2 <- c("El valor p es mayor que el valor de alfa=", input_alfa, ".","<br/>",
                       "No se rechaza la Ho.", "<br/>",
                       "La variables '", BARTLETT$DETALLES$nombre_VR , "' presenta varianzas estadísticamente homogéneas para todos los niveles de '", BARTLETT$DETALLES$nombre_FACTOR, "'.","<br/>")
        ...
        frase2_v4 <- c("El valor p es menor que el valor de alfa=", input_alfa, ".","<br/>",
                       "No se rechaza la Ho.", "<br/>",
                       "Las variables '", BARTLETT$DETALLES$nombre_VR , "' y '", BARTLETT$DETALLES$nombre_FACTOR, "' no presentan varianzas estadísticamente homogéneas.","<br/>")
