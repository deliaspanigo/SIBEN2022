...
PROP01_MASTER <- function(input_datos=NULL, input_exito=1,input_ref=0.5, input_decimales=2, input_alfa=0.05){
  ...
  if (control_general == TRUE) {
    ...
    CONSULTORA <- list()
    CONSULTORA$FA <- FA
    CONSULTORA$FR <- round(FR, input_decimales)
    CONSULTORA$p <- round(p, input_decimales)
    CONSULTORA$N <- N
    CONSULTORA$prop_poblacional <- round(prop_poblacional, input_decimales)
    CONSULTORA$varianza <- round(varianza, input_decimales)
