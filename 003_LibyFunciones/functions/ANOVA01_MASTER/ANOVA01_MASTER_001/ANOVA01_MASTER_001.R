...
ANOVA01_MASTER <- function(input_datos=NULL, input_decimales=2, input_alfa=0.05){
  ...
  if (control_general == TRUE) {
    ...
    if (p_interno_anova < 0.01) p_externo_anova <- "<<0.01" else p_externo_anova <- p_interno_anova
    ...
    # Analisis de los ERRORES
    {
      ...
      if (normalidad == "SI" && homogeneidad == "SI") RESUMEN_NH[1,3] <- "Válidada" else RESUMEN_NH[1,3] <- "Descartada"
