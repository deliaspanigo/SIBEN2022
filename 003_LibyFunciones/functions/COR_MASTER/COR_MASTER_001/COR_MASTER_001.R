...
COR_MASTER <- function(input_datos=NULL, input_decimales=2, input_metodo="all", input_alfa=0.05){
  ...
  # COR de Pearson 
  {
    ...
    PEARSON$estimate <- round(PEARSON$estimate, input_decimales)
    PEARSON$p.value <- round(PEARSON$p.value, input_decimales)
    
    p_interno_pearson <- PEARSON$p.value
    if (p_interno_pearson < 0.01) p_externo_pearson <- "<<0.01" else p_externo_pearson <- p_interno_pearson
  ...
  # COR de SPEARMAN
  {
    ...
    SPEARMAN$estimate <- round(SPEARMAN$estimate, input_decimales)
    SPEARMAN$p.value <- round(SPEARMAN$p.value, input_decimales)
    
    p_interno_spearman <- SPEARMAN$p.value
    if (p_interno_spearman < 0.01) p_externo_spearman <- "<<0.01" else p_externo_spearman <- p_interno_spearman
  ...
  } else   if (CONTROL_nh == FALSE | input_metodo == "Spearman") {
