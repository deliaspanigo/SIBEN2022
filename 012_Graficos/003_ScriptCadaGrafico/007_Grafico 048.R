...
###########################################################################################
# SIBEN 2020
# col_ipt1 <- c("W", "AH", "AS", "BD", "BO", "BZ", "CK", "CV", "DG", "DR") # SIBEN 2020

# SIBEN 2021 - 17/05/2022
col_ipt1 <- c("W", "AH", "AS", "BD", "BO", "BZ", "CK", "CV", "DG") 
############################################################################################
col_ipt2 <- "FC"
col_ipt3 <- "E"
...
for (k in 1:length(score_unidad)) {
  ...
  # SIBEN 202
  # suma2 <- sum(na.omit(mini3$"Egreso_Dias Internac"))    # Suma de Dias de internacion
  #######################################################################################
  
  
  # SIBEN 2021 - 30 de Mayo 2022
  suma2 <- sum(na.omit(mini3[,3]))    # Suma de Dias de internacion
  ##########################################################################################
