...
# SIBEN 2021 - Agregado 17/06/2022
#######################################################################

cambio[,1] <- as.character(cambio[,1])
cambio[,2] <- as.character(cambio[,2])

# colnames(cambio)
for ( k in 1:nrow(cambio)){
  
  if(!is.na(cambio[k,1])) cambio[k,1] <- paste0(cambio[k,1], "- Nac")
  if(!is.na(cambio[k,2])) cambio[k,2] <- paste0(cambio[k,2], "- Alta")
}

armado1 <- na.omit(BASE_NIVELES_RECAT[,colnames(mini)[1]])
armado1 <- armado1[3:length(armado1)]
armado2 <- na.omit(BASE_NIVELES_RECAT[,colnames(mini)[2]])
armado2 <- armado2[3:length(armado2)]
armado3 <- paste0(armado1, "- Nac")
armado4 <- paste0(armado2, "- Alta")

cambio[,1] <- factor(cambio[,1], levels  = armado3)
cambio[,2] <- factor(cambio[,2], levels  = armado4)
##########################################################################
